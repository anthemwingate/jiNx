...
class Transcriber(fluteline.Consumer):
    ...
    def __init__(self, settings, speech_to_text_username, speech_to_text_api_key, speech_to_text_endpoint_url,):
        '''
        ...
        params = {
            'access_token': _request_token(speech_to_text_api_key),
        }
        for param in URL_ONLY_PARAMS:
            if param in settings:
                params[param] = settings.pop(param)
        self._url = URL_TEMPLATE.format(speech_to_text_username) + "?" + urlencode(params)
